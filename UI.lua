--[[
    ...
    NexUI Library — v6.0  (Premium Rewrite)
    
    FEATURES:
      • Smooth lerp drag, Quint/Exponential default easing
      • Slide tab transitions with staggered cascade
      • Two-column section layout (Left/Right)
      • Active background blur
      • 6 theme presets + runtime theme switching
      • Enhanced Slider (pop handle, floating tooltip, glow)
      • Searchable Dropdown
      • Color Picker with HEX input + Alpha channel
      • Tooltips on elements
      • Dialog / Confirmation boxes
      • Optional Watermark (FPS/Ping)
      • Optional Keybind List panel
      • Sub-Folders (expandable groups)
      • Player Select element
      • Animated notifications with sequential entrance
    
    USAGE:
        local Library = loadstring(...)()
        Library:SetTheme("Midnight")
        local Window = Library.new("Hub Name")
        Window:Watermark(true) -- optional
        Window:KeybindList(true) -- optional
        
        local Tab = Window:new_tab("Combat", "⚔")
        local L = Tab:new_section("Aimbot")
        local R = Tab:new_section("Settings", "Right")
        
        L:element("Toggle", "Enabled", {tooltip="Locks onto target"}, cb):add_keybind()
        L:element("Slider", "FOV", {default={min=1,max=360,default=90}}, cb)
        R:element("SearchDropdown", "Target", {options={"Head","Torso"}}, cb)
        R:element("ColorPicker", "Color", {default=Color3.new(1,0,0), alpha=true}, cb)
        R:element("PlayerSelect", "Player", {}, cb)
        
        local folder = L:sub_folder("Advanced")
        folder:element("Toggle", "Prediction", {}, cb)
        
        Window:Dialog("Confirm", "Are you sure?", function() end, function() end)
]]
...
local T={}; for k,v in pairs(Library.Themes.Default) do T[k]=v end

function Library:SetTheme(n)
    local t=Library.Themes[n]; if not t then return end
    Library.CurrentTheme=n; for k,v in pairs(t) do T[k]=v end
    for _,e in ipairs(Library._themed) do
        if e[1] and e[1].Parent then pcall(function() TS:Create(e[1],TweenInfo.new(.4,Enum.EasingStyle.Quint),{[e[2]]=T[e[3]]}):Play() end) end
    end
end
local function Reg(o,p,k) table.insert(Library._themed,{o,p,k}); pcall(function() o[p]=T[k] end); return o end

-- ═══════════════════════════════════════════════════
--  CORE UTILITIES (Quint default easing)
-- ═══════════════════════════════════════════════════
local function Tw(o,p,t,s,d)
    pcall(function() TS:Create(o,TweenInfo.new(t or .2,s or Enum.EasingStyle.Quint,d or Enum.EasingDirection.Out),p):Play() end)
end

local function I(c,p,par)
    local ok,i=pcall(Instance.new,c); if not ok then return nil end
...
local function Shadow(p,z)
    I("ImageLabel",{Name="_Sh",Size=UDim2.new(1,48,1,48),Position=UDim2.new(0,-24,0,-24),
    BackgroundTransparency=1,Image="rbxassetid://5554236805",ImageColor3=Color3.new(0,0,0),
    ImageTransparency=.4,ZIndex=(z or 1)-1,ScaleType=Enum.ScaleType.Slice,SliceCenter=Rect.new(23,23,277,277)},p)
end
...
function Library.new(title, toggleKey)
    ...
    win._watermark=nil; win._kbList=nil

    local W,H=780,490; local TB=46; local IW=46; local SW=150; local GAP=10

    -- ScreenGui
    local gui=I("ScreenGui",{Name="__NexUI_"..math.random(1e4,9e4),ResetOnSpawn=false,
    ...
    -- Blur
    for _,v in ipairs(Light:GetChildren()) do if v.Name=="__NexBlur" then v:Destroy() end end
    ...
    -- Tooltip holder (global)
    win._tooltipLabel=I("TextLabel",{
        Text="",Size=UDim2.new(0,0,0,22),AutomaticSize=Enum.AutomaticSize.X,
        BackgroundColor3=T.Elevated,TextColor3=T.Text,TextSize=10,Font=Enum.Font.Gotham,
        BorderSizePixel=0,Visible=false,ZIndex=999,
    },gui)
    Cn(5,win._tooltipLabel); St(T.BorderLight,1,win._tooltipLabel); Pd(0,0,8,8,win._tooltipLabel)

    -- Main
    local main=I("Frame",{
        Name="W",Size=UDim2.new(0,W*.85,0,H*.85),
        Position=UDim2.new(.5,0,.5,0),AnchorPoint=Vector2.new(.5,.5),
        BackgroundColor3=T.BG,BorderSizePixel=0,ClipsDescendants=true,BackgroundTransparency=.4,
    },gui)
    Cn(12,main); Shadow(main,2); Reg(main,"BackgroundColor3","BG")
    win.Main=main; win._fullSize=UDim2.new(0,W,0,H)

    -- Intro
    task.defer(function() Tw(main,{Size=UDim2.new(0,W,0,H),BackgroundTransparency=0},.55,Enum.EasingStyle.Back) end)
    ...
    -- Accent line (inside topbar, no overflow)
    ...
    do
        ...
        -- Lerp loop for smooth weighted drag
        RS.RenderStepped:Connect(function()
            if dragging then
                local cX=main.Position.X.Offset; local cY=main.Position.Y.Offset
                local tX=targetPos.X.Offset; local tY=targetPos.Y.Offset
                local lerpFactor=.25
                local nX=cX+(tX-cX)*lerpFactor
                local nY=cY+(tY-cY)*lerpFactor
                main.Position=UDim2.new(targetPos.X.Scale,nX,targetPos.Y.Scale,nY)
            end
        end)
    end
    ...
    -- ══ Tooltip system ══
    local tooltipConn
    function win:_showTooltip(text, anchorFrame)
        if not text or text=="" then return end
        win._tooltipLabel.Text=text; win._tooltipLabel.Visible=true
        if tooltipConn then tooltipConn:Disconnect() end
        tooltipConn=RS.RenderStepped:Connect(function()
            local mp=UIS:GetMouseLocation()
            win._tooltipLabel.Position=UDim2.new(0,mp.X+14,0,mp.Y+4)
        end)
    end
    function win:_hideTooltip()
        win._tooltipLabel.Visible=false
    ...
    UIS.InputEnded:Connect(function(inp)
        ...
        for flag,elem in pairs(Library.Elements) do
            if elem._keybind and inp.KeyCode==elem._keybind and elem._keybindMode=="Hold" and elem.Type=="Toggle" and elem._setValue then
...
function Library:Watermark(enabled, customText)
    ...
    if not self._watermark then
        local wm=I("Frame",{
            ...
            Position=UDim2.new(0.5,0,0,8),AnchorPoint=Vector2.new(.5,0),
        ...
        local wmLbl=I("TextLabel",{
            ...
            BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=10,Font=Enum.Font.Gotham,ZIndex=101,
        },wm)
        ...
        RS.RenderStepped:Connect(function()
            ...
            wmLbl.Text=(customText or self.Title).."  |  "..fps.." FPS  |  "..ping.."ms"
        end)
    end
...
-- ═══════════════════════════════════════════════════
--  KEYBIND LIST (optional floating panel)
-- ═══════════════════════════════════════════════════
function Library:KeybindList(enabled)
    ...
    if not self._kbList then
        ...
        I("TextLabel",{Text="KEYBINDS",Size=UDim2.new(1,0,0,18),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=91,LayoutOrder=0},kbl)
        Ls(kbl,3)
        self._kbList=kbl; self._kbListItems={}

        RS.Heartbeat:Connect(function()
            if not kbl.Visible then return end
            -- Clear old
            for _,item in ipairs(self._kbListItems) do item:Destroy() end
            table.clear(self._kbListItems)
            -- Rebuild
            local order=1
            for flag,elem in pairs(Library.Elements) do
                if elem._keybind and elem.Type=="Toggle" and Library.Flags[flag] and Library.Flags[flag].Toggle then
                    local row=I("Frame",{Size=UDim2.new(1,0,0,16),BackgroundTransparency=1,ZIndex=91,LayoutOrder=order},kbl)
                    I("TextLabel",{Text=elem.Name,Size=UDim2.new(1,-35,1,0),BackgroundTransparency=1,TextColor3=T.Text,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=92},row)
                    I("TextLabel",{Text="["..KeyName(elem._keybind).."]",Size=UDim2.new(0,30,1,0),Position=UDim2.new(1,-30,0,0),BackgroundTransparency=1,TextColor3=T.Accent,TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Right,ZIndex=92},row)
                    table.insert(self._kbListItems,row)
                    order=order+1
                end
            end
        end)
...
function Library:Dialog(title, message, onConfirm, onCancel)
    -- Dim overlay
    local dim=I("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.new(0,0,0),BackgroundTransparency=.5,ZIndex=800},self.Overlay)

    local dialog=I("Frame",{
        Size=UDim2.new(0,0,0,0),Position=UDim2.new(.5,0,.5,0),AnchorPoint=Vector2.new(.5,.5),
    ...
    -- Animate in
    task.defer(function()
        Tw(dialog,{Size=UDim2.new(0,320,0,160)},.35,Enum.EasingStyle.Back)
    end)

    -- Content (delayed for clip)
    task.delay(.15,function()
        I("TextLabel",{Text=title or"Confirm",Size=UDim2.new(1,-24,0,22),Position=UDim2.new(0,12,0,14),BackgroundTransparency=1,TextColor3=T.Text,TextSize=14,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=802},dialog)
        I("TextLabel",{Text=message or"",Size=UDim2.new(1,-24,0,0),AutomaticSize=Enum.AutomaticSize.Y,Position=UDim2.new(0,12,0,40),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=11,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,ZIndex=802},dialog)

        local function Close()
            Tw(dialog,{Size=UDim2.new(0,0,0,0),BackgroundTransparency=.5},.2)
            Tw(dim,{BackgroundTransparency=1},.2)
            task.delay(.22,function() pcall(function() dialog:Destroy();dim:Destroy() end) end)
        end

        -- Confirm
        local cfm=I("TextButton",{Text="Confirm",Size=UDim2.new(0,100,0,30),Position=UDim2.new(1,-218,1,-44),BackgroundColor3=T.Accent,TextColor3=Color3.new(1,1,1),TextSize=11,Font=Enum.Font.GothamBold,BorderSizePixel=0,AutoButtonColor=false,ZIndex=802},dialog)
        Cn(6,cfm)
        cfm.MouseButton1Click:Connect(function() Close(); if onConfirm then onConfirm() end end)
        ...
        -- Cancel
        local ccl=I("TextButton",{Text="Cancel",Size=UDim2.new(0,100,0,30),Position=UDim2.new(1,-112,1,-44),BackgroundColor3=T.Hover,TextColor3=T.TextSub,TextSize=11,Font=Enum.Font.GothamBold,BorderSizePixel=0,AutoButtonColor=false,ZIndex=802},dialog)
        Cn(6,ccl); St(T.BorderLight,1,ccl)
        ccl.MouseButton1Click:Connect(function() Close(); if onCancel then onCancel() end end)
...
function Library:new_tab(name, icon)
    ...
    -- Icon button (FIX: indicator centered with AnchorPoint)
    ...
    -- FIX: Indicator centered properly using AnchorPoint
    local ind=I("Frame",{
        Size=UDim2.new(0,3,0,0),
        Position=UDim2.new(0,3,0.5,0),
        AnchorPoint=Vector2.new(0,0.5), -- vertically centered
    ...
    ib.MouseEnter:Connect(function()
        tt.Visible=true
        if not tab.Active then Tw(ib,{BackgroundTransparency=0,BackgroundColor3=T.Hover},.15); Tw(il,{TextColor3=T.TextSub},.15) end
    end)
    ib.MouseLeave:Connect(function()
        tt.Visible=false
        if not tab.Active then Tw(ib,{BackgroundTransparency=.5,BackgroundColor3=T.IconBar},.15); Tw(il,{TextColor3=T.TextMut},.15) end
    end)
    ...
    local colFrame=I("Frame",{Name="Cols",Size=UDim2.new(1,-20,0,0),Position=UDim2.new(0,10,0,10),BackgroundTransparency=1,BorderSizePixel=0,ZIndex=5},page)

    local leftCol=I("Frame",{Name="L",Size=UDim2.new(.5,-GAP/2,0,0),Position=UDim2.new(0,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=5},colFrame)
    Ls(leftCol,GAP)
    local rightCol=I("Frame",{Name="R",Size=UDim2.new(.5,-GAP/2,0,0),Position=UDim2.new(.5,GAP/2,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=5},colFrame)
    Ls(rightCol,GAP)

    tab.LeftCol=leftCol; tab.RightCol=rightCol; tab.ColFrame=colFrame

    local function UpdCanvas()
        local lH=leftCol:FindFirstChildOfClass("UIListLayout").AbsoluteContentSize.Y
        local rH=rightCol:FindFirstChildOfClass("UIListLayout").AbsoluteContentSize.Y
        colFrame.Size=UDim2.new(1,-20,0,math.max(lH,rH))
        page.CanvasSize=UDim2.new(0,0,0,math.max(lH,rH)+30)
    end
    leftCol:FindFirstChildOfClass("UIListLayout"):GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdCanvas)
    rightCol:FindFirstChildOfClass("UIListLayout"):GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdCanvas)
    ...
    function tab:new_section(sectionName, side)
        ...
        subE.MouseButton1Click:Connect(function()
            task.defer(function() if section.Frame then
        ...
        -- Card
        local card=I("Frame",{Name=sectionName,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundColor3=T.Card,BorderSizePixel=0,ZIndex=6},targetCol)
        Cn(10,card); St(T.Border,1,card); Ls(card,0); Reg(card,"BackgroundColor3","Card")
        section.Frame=card

        -- Stagger entrance
        card.BackgroundTransparency=.6; card.Position=UDim2.new(0,0,0,12)
        task.delay(#tab.Sections*.07,function() Tw(card,{BackgroundTransparency=0,Position=UDim2.new(0,0,0,0)},.45) end)
        ...
        local pill=I("Frame",{Size=UDim2.new(0,3,0,12),Position=UDim2.new(0,8,.5,-6),BackgroundColor3=T.Accent,BorderSizePixel=0,ZIndex=8},hd);Cn(2,pill);Reg(pill,"BackgroundColor3","Accent")
        ...
        Ls(ec,1); Pd(5,7,7,7,ec); section.Container=ec

        -- ═══════════════════════════════
        --  SUB-FOLDER
        -- ═══════════════════════════════
        function section:sub_folder(folderName)
            local fRow=I("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=7,ClipsDescendants=true},ec)

            -- Header button
            local fBtn=I("TextButton",{Text="",Size=UDim2.new(1,0,0,28),BackgroundColor3=T.Elevated,BackgroundTransparency=.5,BorderSizePixel=0,AutoButtonColor=false,ZIndex=8},fRow)
            Cn(5,fBtn)
            local fArrow=I("TextLabel",{Text="▸",Size=UDim2.new(0,14,1,0),Position=UDim2.new(0,6,0,0),BackgroundTransparency=1,TextColor3=T.TextMut,TextSize=10,Font=Enum.Font.GothamBold,ZIndex=9},fBtn)
            I("TextLabel",{Text=folderName,Size=UDim2.new(1,-24,1,0),Position=UDim2.new(0,20,0,0),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=10,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=9},fBtn)

            -- Content container (starts hidden)
            local fContent=I("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=7,Visible=false},fRow)
            ...
            fBtn.MouseLeave:Connect(function() if not fOpen then Tw(fBtn,{BackgroundTransparency=.5,BackgroundColor3=T.Elevated},.1) end end)

            -- Return element factory that targets fContent
        ...
        function section:element(eType,eName,options,callback,targetContainer)
            ...
            local function Row(h)
                local f=I("Frame",{Size=UDim2.new(1,0,0,h or 34),BackgroundTransparency=1,BorderSizePixel=0,ZIndex=7},container); Cn(6,f)
                -- Tooltip support
                if tooltipText and tooltipText~="" then
                    local hoverBtn=I("TextButton",{Text="",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=14},f)
                    local tooltipTimer
                    hoverBtn.MouseEnter:Connect(function()
                        tooltipTimer=task.delay(.8,function() win:_showTooltip(tooltipText,f) end)
                    end)
                    hoverBtn.MouseLeave:Connect(function()
                        if tooltipTimer then task.cancel(tooltipTimer) end; win:_hideTooltip()
                    end)
                end
                return f
            end
            ...
            if eType=="Toggle" then
                Library.Flags[flag]={Toggle=false,Active=false}; local row=Row(34)
                ...
                local sw=I("Frame",{Size=UDim2.new(0,34,0,17),Position=UDim2.new(1,-42,.5,-8),BackgroundColor3=T.ToggleBg,BorderSizePixel=0,ZIndex=9},row);Cn(9,sw);St(T.BorderLight,1,sw)
                ...
                local tog=false
                local function Set(v,s) tog=v;Library.Flags[flag].Toggle=v;Library.Flags[flag].Active=v
                    if v then Tw(sw,{BackgroundColor3=T.Accent},.22);Tw(kn,{Position=UDim2.new(0,20,.5,-5),BackgroundColor3=Color3.new(1,1,1)},.22,Enum.EasingStyle.Back);Tw(gl,{BackgroundTransparency=.78},.35)
                    else Tw(sw,{BackgroundColor3=T.ToggleBg},.22);Tw(kn,{Position=UDim2.new(0,3,.5,-5),BackgroundColor3=T.ToggleKnob},.22,Enum.EasingStyle.Back);Tw(gl,{BackgroundTransparency=1},.22) end
                    if not s then pcall(callback,{Toggle=v}) end
                end
                ...
                ck.MouseEnter:Connect(function() Tw(hov,{BackgroundTransparency=.88},.12) end)
                ck.MouseLeave:Connect(function() Tw(hov,{BackgroundTransparency=1},.12) end)
            ...
            -- ═══ SLIDER (enhanced: pop handle, floating tooltip, glow) ═══
            elseif eType=="Slider" then
                local c=options.default or{}; local mn,mx,df,dc=c.min or 0,c.max or 100,c.default or(c.min or 0),c.decimals or 0
                ...
                -- Fill glow (visible during drag)
                local fillGlow=St(T.AccentGlow,1,fl); fillGlow.Transparency=1
                ...
                -- Floating tooltip
                local ftip=I("TextLabel",{Text="",Size=UDim2.new(0,0,0,18),AutomaticSize=Enum.AutomaticSize.X,BackgroundColor3=T.Elevated,TextColor3=T.Text,TextSize=10,Font=Enum.Font.GothamBold,BorderSizePixel=0,Visible=false,ZIndex=20},row)
                ...
                local function PH() local p=(Library.Flags[flag].Slider-mn)/(mx-mn); hd2.Position=UDim2.new(0,tr.AbsolutePosition.X-row.AbsolutePosition.X+p*tr.AbsoluteSize.X-6,0,25)
                    ftip.Position=UDim2.new(0,tr.AbsolutePosition.X-row.AbsolutePosition.X+p*tr.AbsoluteSize.X-15,0,10)
                end
                local function SS(v,s) local f=10^dc;local r=math.clamp(math.floor(v*f+.5)/f,mn,mx);Library.Flags[flag].Slider=r;fl.Size=UDim2.new((r-mn)/(mx-mn),0,1,0);vl.Text=Fmt(r);ftip.Text=Fmt(r);PH();if not s then pcall(callback,{Slider=r}) end end
                ...
                tc.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then
                    dr=true; ftip.Visible=true
                    -- Pop handle bigger
                    Tw(hd2,{Size=UDim2.new(0,16,0,16)},.12,Enum.EasingStyle.Back)
                    Tw(fillGlow,{Transparency=.3},.15)
                    -- First click: tween to position
                    local pct=math.clamp((i.Position.X-tr.AbsolutePosition.X)/math.max(tr.AbsoluteSize.X,1),0,1)
                    SS(mn+pct*(mx-mn))
                end end)
                UIS.InputChanged:Connect(function(i) if dr and i.UserInputType==Enum.UserInputType.MouseMovement then
                    SS(mn+math.clamp((i.Position.X-tr.AbsolutePosition.X)/math.max(tr.AbsoluteSize.X,1),0,1)*(mx-mn))
                end end)
                UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 and dr then
                    dr=false; ftip.Visible=false
                    Tw(hd2,{Size=UDim2.new(0,12,0,12)},.15,Enum.EasingStyle.Back)
            ...
            -- ═══ SEARCHABLE DROPDOWN ═══
            elseif eType=="SearchDropdown" then
                local opts=options.options or{"Option"};local dO=options.default or opts[1]
                Library.Flags[flag]={Dropdown=dO};local row=Row(52)
                I("TextLabel",{Text=eName,Size=UDim2.new(1,-8,0,16),Position=UDim2.new(0,8,0,2),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=9},row)
                local df=I("Frame",{Size=UDim2.new(1,-16,0,26),Position=UDim2.new(0,8,0,20),BackgroundColor3=T.Elevated,BorderSizePixel=0,ZIndex=9},row);Cn(6,df);St(T.BorderLight,1,df)
                ...
                local function SD(v,s) Library.Flags[flag].Dropdown=v;sel.Text=v;if not s then pcall(callback,{Dropdown=v}) end end
                elem._setValue=SD
                I("TextButton",{Text="",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=11},df).MouseButton1Click:Connect(function()
                    Tw(ch,{Rotation=180},.2)
                    win:_openSearchDropdown(opts,df,function(v,s) SD(v,s);Tw(ch,{Rotation=0},.2) end,function() Tw(ch,{Rotation=0},.2) end)
                end)
                elem._row=row; function elem:set_value(v) SD(v) end

            -- ═══ PLAYER SELECT ═══
            elseif eType=="PlayerSelect" then
                Library.Flags[flag]={Dropdown=""}; local row=Row(52)
                I("TextLabel",{Text=eName,Size=UDim2.new(1,-8,0,16),Position=UDim2.new(0,8,0,2),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=9},row)
                local df=I("Frame",{Size=UDim2.new(1,-16,0,26),Position=UDim2.new(0,8,0,20),BackgroundColor3=T.Elevated,BorderSizePixel=0,ZIndex=9},row);Cn(6,df);St(T.BorderLight,1,df)
                local sel=I("TextLabel",{Text="Select Player",Size=UDim2.new(1,-28,1,0),Position=UDim2.new(0,8,0,0),BackgroundTransparency=1,TextColor3=T.TextSub,TextSize=11,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=10},df)
                local function SD(v,s) Library.Flags[flag].Dropdown=v;sel.Text=v;sel.TextColor3=T.Text;if not s then pcall(callback,{Dropdown=v}) end end
                elem._setValue=SD
                I("TextButton",{Text="",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=11},df).MouseButton1Click:Connect(function()
                    local names={}; for _,p in ipairs(Players:GetPlayers()) do if p~=LP then table.insert(names,p.Name) end end
                    win:_openDropdown(names,df,function(v,s) SD(v,s) end)
                end)
                elem._row=row

            -- ═══ COMBO ═══
            elseif eType=="Combo" then
                local opts=options.options or{};local dS=options.default or{};local maxSel=options.max_selections
                ...
                for _,opt in ipairs(opts) do
                    ...
                    I("TextButton",{Text="",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=12},cr).MouseButton1Click:Connect(function()
                        ...
                        if idx then table.remove(cur,idx);ck.Visible=false;Tw(cb,{BackgroundColor3=T.ToggleBg},.12)
                            ...
                            if maxSel and #cur>=maxSel then return end
                            table.insert(cur,opt);ck.Visible=true;Tw(cb,{BackgroundColor3=T.Accent},.12)
                        end; pcall(callback,{Combo=cur})
                    end)
                end; elem._row=row
            ...
            elseif eType=="TextBox" then
                ...
                elem._row=row;function elem:set_value(v) tb.Text=tostring(v);Library.Flags[flag].Text=tostring(v) end

            -- ═══ COLOR PICKER (with HEX + Alpha) ═══
            elseif eType=="ColorPicker" then
                local dC=options.default or Color3.fromRGB(255,0,0); local hasAlpha=options.alpha or false
                local alpha=options.defaultAlpha or 1
                Library.Flags[flag]={Color=dC,Alpha=alpha}
                ...
                local function UC(si) local c=Color3.fromHSV(h,s,v);Library.Flags[flag].Color=c;Library.Flags[flag].Alpha=alpha;pv.BackgroundColor3=c;if not si then pcall(callback,{Color=c,Alpha=alpha}) end end
                ...
                pv.MouseButton1Click:Connect(function()
                    ...
                    local PW=hasAlpha and 230 or 210; local PH=hasAlpha and 210 or 185
                    ...
                    local hCur=I("Frame",{Size=UDim2.new(1,4,0,4),Position=UDim2.new(0,-2,h,-2),BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,ZIndex=602},hB);Cn(2,hCur)

                    -- HEX input
                    local hexBox=I("TextBox",{Text=Color3ToHex(Color3.fromHSV(h,s,v)),Size=UDim2.new(0,70,0,20),Position=UDim2.new(0,8,0,SS+30),BackgroundColor3=T.Panel,TextColor3=T.Text,PlaceholderColor3=T.TextDis,TextSize=10,Font=Enum.Font.Gotham,ClearTextOnFocus=true,BorderSizePixel=0,ZIndex=601},pf)
                    Cn(4,hexBox);St(T.BorderLight,1,hexBox);Pd(0,0,6,6,hexBox)
                    I("TextLabel",{Text="HEX",Size=UDim2.new(0,25,0,20),Position=UDim2.new(0,82,0,SS+30),BackgroundTransparency=1,TextColor3=T.TextMut,TextSize=8,Font=Enum.Font.GothamBold,ZIndex=601},pf)

                    hexBox.FocusLost:Connect(function()
                        local c=HexToColor3(hexBox.Text)
                        if c then h,s,v=Color3.toHSV(c);sc.Position=UDim2.new(s,-5,1-v,-5);hCur.Position=UDim2.new(0,-2,h,-2);svF.BackgroundColor3=Color3.fromHSV(h,1,1);UC() end
                    end)

                    -- Alpha slider (optional)
                    local aDrag=false
                    if hasAlpha then
                        I("TextLabel",{Text="ALPHA",Size=UDim2.new(0,35,0,20),Position=UDim2.new(0,8,0,SS+54),BackgroundTransparency=1,TextColor3=T.TextMut,TextSize=8,Font=Enum.Font.GothamBold,ZIndex=601},pf)
                        local aTrack=I("Frame",{Size=UDim2.new(0,SS,0,8),Position=UDim2.new(0,48,0,SS+58),BackgroundColor3=T.SliderTrack,BorderSizePixel=0,ZIndex=601},pf);Cn(4,aTrack)
                        local aFill=I("Frame",{Size=UDim2.new(alpha,0,1,0),BackgroundColor3=T.Accent,BorderSizePixel=0,ZIndex=602},aTrack);Cn(4,aFill)
                        I("TextButton",{Text="",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=603},aTrack).InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then aDrag=true end end)
                        UIS.InputChanged:Connect(function(i) if aDrag and i.UserInputType==Enum.UserInputType.MouseMovement then
                            alpha=math.clamp((i.Position.X-aTrack.AbsolutePosition.X)/math.max(aTrack.AbsoluteSize.X,1),0,1)
                            aFill.Size=UDim2.new(alpha,0,1,0); UC()
                        end end)
                        UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then aDrag=false end end)
                    end
                    ...
                    UIS.InputChanged:Connect(function(i) if i.UserInputType~=Enum.UserInputType.MouseMovement then return end
                        if sD then s=math.clamp((i.Position.X-svF.AbsolutePosition.X)/SS,0,1);v=1-math.clamp((i.Position.Y-svF.AbsolutePosition.Y)/SS,0,1);sc.Position=UDim2.new(s,-5,1-v,-5);hexBox.Text=Color3ToHex(Color3.fromHSV(h,s,v));UC()
                        elseif hD then h=math.clamp((i.Position.Y-hB.AbsolutePosition.Y)/SS,0,1);hCur.Position=UDim2.new(0,-2,h,-2);svF.BackgroundColor3=Color3.fromHSV(h,1,1);hexBox.Text=Color3ToHex(Color3.fromHSV(h,s,v));UC() end
                    end)
                    ...
                    oC=UIS.InputBegan:Connect(function(inp) if not rdy or sD or hD or aDrag then return end;if inp.UserInputType==Enum.UserInputType.MouseButton1 then task.defer(function() if pf and pf.Parent and IsOut(inp.Position,pf) then CP() end end) end end)
                end)
...
function Library:SelectTab(target)
    ...
    if self.ActiveTab and self.ActiveTab~=target and self.ActiveTab.ColFrame then
        ...
        task.delay(.18,function() old.Page.Visible=false; old.ColFrame.Position=UDim2.new(0,10,0,10) end)
    end
...
-- ═══════════════════════════════════════════════════
--  POPUPS (Keybind, Dropdown, SearchDropdown)
-- ═══════════════════════════════════════════════════
...
function Library:_openDropdown(opts,anchor,setFn,onClose)
    ...
    task.defer(function() local ap=anchor.AbsolutePosition;local as=anchor.AbsoluteSize;dd.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+3);Tw(dd,{Size=UDim2.new(0,as.X,0,tH)},.25,Enum.EasingStyle.Back) end)
...
-- Searchable Dropdown
function Library:_openSearchDropdown(opts,anchor,setFn,onClose)
    if self._dropdown then self._dropdown:Destroy();self._dropdown=nil;if onClose then onClose() end;return end
    local IH=26; local MX=7; local tH=math.min(#opts,MX)*IH+40
    local dd=I("Frame",{Size=UDim2.new(0,anchor.AbsoluteSize.X,0,0),BackgroundColor3=T.Elevated,BorderSizePixel=0,ZIndex=600,ClipsDescendants=true},self.Overlay)
    Cn(7,dd);St(T.BorderLight,1,dd);Shadow(dd,600);self._dropdown=dd
    task.defer(function() local ap=anchor.AbsolutePosition;local as=anchor.AbsoluteSize;dd.Position=UDim2.new(0,ap.X,0,ap.Y+as.Y+3);Tw(dd,{Size=UDim2.new(0,as.X,0,tH)},.25,Enum.EasingStyle.Back) end)

    -- Search box inside dropdown
    local sBox=I("TextBox",{Text="",PlaceholderText="Search...",Size=UDim2.new(1,-10,0,22),Position=UDim2.new(0,5,0,5),BackgroundColor3=T.Panel,TextColor3=T.Text,PlaceholderColor3=T.TextDis,TextSize=10,Font=Enum.Font.Gotham,ClearTextOnFocus=false,BorderSizePixel=0,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=602},dd)
    Cn(4,sBox);St(T.Border,1,sBox);Pd(0,0,6,6,sBox)

    local scr=I("ScrollingFrame",{Size=UDim2.new(1,0,1,-30),Position=UDim2.new(0,0,0,30),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=2,ScrollBarImageColor3=T.Accent,CanvasSize=UDim2.new(0,0,0,#opts*IH+10),ZIndex=601},dd)
    ...
    for _,opt in ipairs(opts) do
        ...
        it.MouseLeave:Connect(function() Tw(it,{BackgroundTransparency=1},.08) end)
        it.MouseButton1Click:Connect(function() setFn(opt,false);Tw(dd,{Size=UDim2.new(0,dd.AbsoluteSize.X,0,0)},.15);task.delay(.16,function() if dd and dd.Parent then dd:Destroy() end;self._dropdown=nil end) end)
        table.insert(items,{btn=it,name=string.lower(opt)})
    end

    sBox:GetPropertyChangedSignal("Text"):Connect(function()
        local q=string.lower(sBox.Text)
        for _,item in ipairs(items) do
            item.btn.Visible=q=="" or string.find(item.name,q,1,true)~=nil
        end
    end)
...
function Library:SetOpen(state)
    self.Open=state
    if state then
        self.Main.Visible=true;self.Main.Size=UDim2.new(0,self._fullSize.X.Offset*.88,0,self._fullSize.Y.Offset*.88);self.Main.BackgroundTransparency=.3
        Tw(self.Main,{Size=self._fullSize,BackgroundTransparency=0},.4,Enum.EasingStyle.Back)
        Tw(self._blur,{Size=8},.5)
    else
        Tw(self.Main,{Size=UDim2.new(0,self._fullSize.X.Offset*.9,0,self._fullSize.Y.Offset*.9),BackgroundTransparency=.25},.22,Enum.EasingStyle.Exponential,Enum.EasingDirection.In)
...
function Library:LoadConfig(name) pcall(function()
    ...
    for f,v in pairs(d) do for k,x in pairs(v) do if type(x)=="table" and x._c3 then v[k]=Color3.new(x.R,x.G,x.B) end end
        Library.Flags[f]=v;local e=Library.Elements[f];if e and e._setValue then
            ...
            if e.Type=="Dropdown" or e.Type=="SearchDropdown" or e.Type=="PlayerSelect" then pcall(e._setValue,v.Dropdown,true) end
...
function Library:Notify(title,desc,duration,nType)
    ...
    local nf=I("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,Position=UDim2.new(1.2,0,0,0),BackgroundColor3=T.Panel,BorderSizePixel=0,BackgroundTransparency=.3,ZIndex=10,ClipsDescendants=true},wr);Cn(10,nf);St(T.Border,1,nf)
    ...
    -- Entry
    task.defer(function() Tw(nf,{Position=UDim2.new(0,0,0,0)},.5) end)
    ...
    task.delay(.18,function() Tw(tL,{TextTransparency=0},.3) end)
    task.delay(.26,function() if dL then Tw(dL,{TextTransparency=.1},.3) end end)
